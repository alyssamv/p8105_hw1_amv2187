...
The number of observations (rows) and variables (columns) in this data frame is (`r dim(p2_df)`), respectively. The mean of x is `r mean(p2_df$x)`; the median of x is `r median(p2_df$x)`. The proportions of observations for which `x + y > 0` is `r sum(p2_df$num_vec)/n`.
